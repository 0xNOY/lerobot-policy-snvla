...
@dataclass
class SNVLADatasetRecordConfig(DatasetRecordConfig):
    # Optional remapping used by downstream SNVLA processors when datasets were recorded
    # with non-standard observation names.
...
def add_snvla_recording_features(
...
def fill_narration_frame_defaults(frame: dict, dataset_features: dict) -> None:
    if CURRENT_NARRATION in dataset_features and CURRENT_NARRATION not in frame:
        frame[CURRENT_NARRATION] = ""
    if PREVIOUS_NARRATIONS in dataset_features and PREVIOUS_NARRATIONS not in frame:
        frame[PREVIOUS_NARRATIONS] = json.dumps([])


def log_visualization_narrations(
    display_mode: str,
...
def init_snvla_keyboard_listener(narration_enabled: bool):
    events = {
        "exit_early": False,
    ...
    def on_key(name: str) -> None:
        key = name.lower()
        if key == "right" or (key == "n" and not narration_enabled):
            apply_recording_control("right", events)
        elif key == "left" or (key == "r" and not narration_enabled):
            apply_recording_control("left", events)
        elif key in ("esc", "q"):
            apply_recording_control("esc", events)
        elif key == "n" and narration_enabled:
            events["narration_occurred"] = True
        elif key == "t":
            events["change_task"] = True

    controls_help = "Right/Left/Esc, q=quit, t=task"
    if narration_enabled:
        controls_help += ", n=narration"
    else:
        controls_help += ", r=re-record"
...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        if dataset is not None:
            ...
            fill_narration_frame_defaults(frame, dataset.features)
...
def record(
    ...
    add_snvla_recording_features(dataset_features, has_narrations=narration_manager.is_enabled())
    ...
    try:
        ...
        listener, events = init_snvla_keyboard_listener(narration_enabled=narration_manager.is_enabled())
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                if events.get("change_task", False):
                    ...
                    if new_task.strip():
                        ...
                        logging.info("Task updated to: %s", cfg.dataset.single_task)
                    else:
