...
from lerobot_policy_snvla.constants import STATE_DROPOUT_MASK, TRAINING_EPOCH
...
def make_molmoact2_benchmark_trial(
    ...
    if config.state_dropout_enabled and not bool(dropout.any()):
        raise RuntimeError("Benchmark state_dropout_ratio>0 must execute at least one hidden-state VLM forward.")
    batch["__benchmark__"] = {
        "dataset_root": str(dataset_root),
